...
// Dev runs the engine from the repo venv as `python -m nojohns_engine`;
// packaged builds use the PyInstaller exe shipped in resources/engine.
function resolveEngine(): EngineCommand {
  if (app.isPackaged) {
    return {
      command: join(process.resourcesPath, 'engine', 'nojohns-engine.exe'),
      baseArgs: [],
